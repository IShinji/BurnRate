  ...
  const toolStats = nonZeroTools.slice(0, 4).map((tool, index) => {
    ...
    return `
      <g transform="translate(${x}, ${y})">
        ...
        <text x="45" y="29" class="tool-name">${xml(tool.name)}</text>
        <text x="45" y="55" class="tool-metric">${xml(formatCompact(tool.tokens))} tokens</text>
        <text x="45" y="78" class="tool-muted">spent ${xml(formatUsd(tool.costUSD))}</text>
...
function renderLineChart(daily, options) {
  ...
  const grid = [0, 0.25, 0.5, 0.75, 1].map(p => {
    ...
    return `<line x1="${x}" y1="${gy}" x2="${x + width}" y2="${gy}" stroke="#E2E8F0" stroke-width="1"/><text x="${x + width + 15}" y="${gy + 4}" class="chart-label">${val}</text>`;
  }).join('');
